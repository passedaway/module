...
#endif
	hd1650_show(g_hd1650.disp_buf, g_dot_flag);
	/* this is for test timer ,if set this over,just write one time ,then it will all errors!*/
    up( &mysem );
...
static __init int fp_init(void)
{
	...
	do_gettimeofday(&tv);
	if( tv.tv_sec > 3600 )
		g_hd1650.time.hh = tv.tv_sec / 60 / 60;
	if( tv.tv_sec > 60)
